...
def train_validate(G, D, G_optim, D_optim, loader, epoch, is_train):
    ...
    for batch_idx, (x, _) in enumerate(data_loader):
        ...
        recon_loss = mse_loss(x_fake.view(-1, 1), x.view(-1, 1))
        g_loss = loss_bce(y_fake, y_ones) + recon_loss
        G_batch_loss += g_loss.item() / batch_size
        recon_batch_loss += recon_loss.item() / batch_size
    ...
    # print('\n Avg mse loss:{}'.format(recon_batch_loss / (batch_idx + 1)))
...
def execute_graph(G, D, G_optim, D_optim, loader, epoch, use_tb):
    ...
    # Generate examples
        img_shape = loader.img_shape
        sample = dcgan_generation_example(G, args.noise_dim, 10, img_shape, args.cuda)
